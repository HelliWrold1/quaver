...
func newVideo(db *gorm.DB, opts ...gen.DOOption) video {
	...
	_video.Datetime = field.NewTime(tableName, "datetime")
...
func (v *video) updateTableName(table string) *video {
	...
	v.Datetime = field.NewTime(table, "datetime")
...
func (v *video) fillFieldMap() {
	...
	v.fieldMap["datetime"] = v.Datetime
}
...
func (v videoDo) FilterWithNameAndRole(name string, role string) (result []model.Video, err error) {
	...
	generateSQL.WriteString("SELECT * FROM video WHERE name = ? ")
	if role != "" {
